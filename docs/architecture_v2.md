...
핵심 다섯 테이블(`raw` / `clean` / `feature` / `model_registry` / `prediction_result`)의 **역할은 바꾸지 않는다**.

- **clean**: `target_default_yn` = Y/N만.
- **feature**: 공식 모델 입력 = **`model_input_json`** (`features`는 동일 스냅샷·호환용).
- **prediction_result**: 공식 결과 = **`risk_score`**, **`predicted_default_yn`**, **`risk_grade`**.
- 이후 스키마 변경은 **컬럼 추가** 또는 **`sql/NNN_*.sql` 마이그레이션**만 (`sql/README.md`).
...
- **있는 것**: `app/core`, `models`, `schemas`, `routers`(health·predictions), `services`(ingestion, preprocessing, feature, model, prediction), `repositories`, `scripts/load_raw_data.py`, `build_features.py`, `train_model.py`, `sql/`, `ml/artifacts/`(바이너리는 `.gitignore`로 제외 가능).
- **PRD 대비 비어 있거나 얇은 것**: `policy_engine_service`, `explanation_service`, `llm_review_service`, 공통 응답 포맷, `docker-compose.yml` 등.
